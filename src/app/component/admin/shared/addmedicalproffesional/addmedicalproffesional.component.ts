...
@Component({
  selector: 'app-addmedicalproffesional',
  templateUrl: './addmedicalproffesional.component.html',
  styleUrls: ['./addmedicalproffesional.component.css']
})
export class AddmedicalproffesionalComponent implements OnInit {
...
addclient() {
  ...
  this.loginForm.value.profile = this.url;
      ...
      this.service.Addproffesional(this.loginForm.value).subscribe({
          next: (res) => {
              console.log("res",res)
              if (res.success) {
...
cancel(){
  this.router.navigate(['/Admin/Medical Professionals'])
}
