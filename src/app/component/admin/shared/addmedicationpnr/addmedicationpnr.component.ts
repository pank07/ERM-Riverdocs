...
@Component({
  selector: 'app-addmedicationpnr',
  templateUrl: './addmedicationpnr.component.html',
  styleUrls: ['./addmedicationpnr.component.css']
})
export class AddmedicationpnrComponent {
  loginForm!: FormGroup;
  constructor(
  ...
  addmeds() {
    if (this.loginForm.invalid) {
      ...
      this.service.addgetprnmedicine(this.loginForm.value).subscribe({
        next: (res) => {
          console.log("res", res)
          this.swet.SucessToast('Medicine added succesfully')
            this.router.navigate(['/Admin/MedicationPRN'])
        },
        error: (err) => {
          console.log(err);
        }
