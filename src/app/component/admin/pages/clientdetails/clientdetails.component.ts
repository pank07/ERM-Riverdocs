...
export class ClientdetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.clientDataid = this.activated.snapshot.paramMap.get('id');
    ...
    if (this.faceSheetForm.invalid) {
      ...
      this.service.postfacesheets(this.faceSheetForm.value).subscribe({
        next: (res) => {
          ...
          this.route.navigate(['/Admin/Clientdetails'])
        },
        error: (err) => {
          console.log(err);
        }
  ...
  updateMedicineInventory(medicine_id: any) {
    ...
    this.service.medicineinventoryadds(medicine_id, data).subscribe({
      next: (res: any) => {
        ...
        this.route.navigate(['/Admin/medication']);
      },
      error: (err) => {
  ...
  byId(id: any): void {
    ...
    this.service.task_by_id(id).subscribe(
      (res: any) => {
        ...
        const taskData = res[0]; // Use the first object in the array
