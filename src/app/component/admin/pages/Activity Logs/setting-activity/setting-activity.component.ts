...
@Component({
  ...
  styleUrls: ['./setting-activity.component.css']
})
export class SettingActivityComponent implements OnInit {
  ...
  getActivityData(): void {
    this.api.getSettingactivity().subscribe((res: any) => {
  ...
  downloadPDF() {
    ...
    html2canvas(element).then((canvas) => {
      ...
      pdf.save('setting-activities.pdf'); // Save the PDF
    });
  }
  ...
  downloadCSV() {
    ...
    link.href = url;
    link.setAttribute('download', 'setting-activities.csv');
    link.click();
