...
export class TaskActivityComponent implements OnInit {
  ...
  getActivityData(): void {
    this.api.gettaskactivity().subscribe((res: any) => {
  ...
  downloadPDF() {
    ...
    html2canvas(element).then((canvas) => {
      ...
      pdf.save('task-activities.pdf'); // Save the PDF
    });
  }
    ...
    downloadCSV() {
      ...
      const rows = this.displayedData.map(activity => [
        ...
        activity.status,
        activity.parsedMessage?.priority || 'Unknown',
      ...
      link.href = url;
      link.setAttribute('download', 'task-activities.csv');
      link.click();
