...
@Component({
  ...
  styleUrls: ['./comment-activity.component.css']
})
export class CommentActivityComponent implements OnInit {
  ...
  getActivityData(): void {
    this.api.getcommentactivity().subscribe((res: any) => {
  ...
  downloadPDF() {
    ...
    html2canvas(element).then((canvas) => {
      ...
      pdf.save('comment-activities.pdf'); // Save the PDF
    });
  }
  
  downloadCSV() {
    const headers = ['Name', 'Status', 'Comment', 'Created At', 'Updated At'];
    const rows = this.displayedData.map(activity => [
      activity.first_name.last_name,
      activity.status,
      activity.parsedMessage?.comment?.toString() || '0',
    ...
    link.href = url;
    link.setAttribute('download', 'comment-activities.csv');
    link.click();
